...
import { SiReact, SiJavascript, SiFigma, SiTailwindcss, SiAdobeillustrator } from 'react-icons/si'
import { useTranslation } from '../../features/i18n/i18n'
import Meta from '../../seo/Meta'
import { buildCreativeWorkSchema } from '../../seo/structuredData'
import { siteMetadata } from '../../config/siteMetadata'

const nutriImages = import.meta.glob<{ default: string }>(
  '../../assets/images/*nutriclaramendes*.{png,jpg,jpeg,webp}',
  { eager: true }
)

const sortedKeys = Object.keys(nutriImages).sort()
const desktopGallery: string[] = []
const mobileGallery: string[] = []

for (const key of sortedKeys) {
  const source = nutriImages[key]?.default
...
function NutriClaraMendesProject() {
  const { t } = useTranslation()
  const content = t('projectDetails.nutriClaraMendes', TRANSLATION_OPTIONS) as NutriContent
  const sections = content.sections ?? []
  const description = content.seo.description
  const pagePath = '/portfolio/nutri-clara-mendes'
...
export default NutriClaraMendesProject
