...
import { SiReact, SiJavascript, SiTailwindcss, SiFigma, SiGooglesearchconsole, SiGoogleanalytics } from 'react-icons/si'
import { useTranslation } from '../../features/i18n/i18n'
import Meta from '../../seo/Meta'
import { buildCreativeWorkSchema } from '../../seo/structuredData'
import { siteMetadata } from '../../config/siteMetadata'

const lexPrimeImages = import.meta.glob<{ default: string }>(
  '../../assets/images/*lexprime*.{png,jpg,jpeg,webp}',
  { eager: true }
)

const lexPrimeKeys = Object.keys(lexPrimeImages).sort()
const desktopGallery: string[] = []
const mobileGallery: string[] = []

for (const key of lexPrimeKeys) {
  const source = lexPrimeImages[key]?.default
...
const skills = [
  ...
  { name: 'Analytics', icon: SiGoogleanalytics, color: '#FF6D00' },
] as const
...
function LexPrimeAdvocaciaProject() {
  const { t } = useTranslation()
  const content = t('projectDetails.lexPrimeAdvocacia', TRANSLATION_OPTIONS) as LexPrimeContent
  const sections = content.sections ?? []
  const description = content.seo.description
  const pagePath = '/projects/lexprime-advocacia'
              ...
              {sections.map((section, index) => {
                ...
                if (isBulletedSection(section)) {
                  return (
                    <div key={`${section.title}-${index}`}>
                      <h2 className="text-xl font-semibold text-white">{section.title}</h2>
                      <ul className="mt-4 space-y-3 pl-6 text-slate-300 marker:text-yellow-300 list-disc">
                ...
                if (isOrderedSection(section)) {
                  return (
                    <div key={`${section.title}-${index}`} className="space-y-4">
                      <h2 className="text-xl font-semibold text-white">{section.title}</h2>
                      <ol className="mt-4 space-y-6 pl-6 text-slate-300 marker:text-yellow-300 list-decimal">
                        {section.items.map((item, itemIndex) => (
                          <li key={`${item.title}-${itemIndex}`}>
                            ...
                            {item.bullets?.length ? (
                              <ul className="mt-3 space-y-2 pl-6 text-slate-300 marker:text-yellow-300 list-disc">
...
export default LexPrimeAdvocaciaProject
